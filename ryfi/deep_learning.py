...
class DQNetwork(nn.Module):
    def __init__(self, board_size, output_size):
        ...
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=32, kernel_size=3, stride=1, padding=1)  # Input is (1, board_size, board_size)
    ...
    def forward(self, x):
        """
        :param x: Input tensor of shape (batch_size, board_size * board_size) which is reshaped into (batch_size, 1, board_size, board_size)
        :return: Output tensor representing Q-values for each action
        """
        # Reshape input to (batch_size, 1, board_size, board_size) for Conv2D layers
        ...
        # Apply fully connected layers with ReLU and Layer Normalization
...
class DQNAgent:
    def __init__(self, state_size, action_size, gamma=0.99, lr=0.0001, batch_size=16, memory_size=10000, epsilon=1.0,
                 epsilon_decay=0.997, epsilon_min=0.01, temperature=1.0):
    ...
    def act(self, state, env, override_random=False):
        ...
        if random.random() < self.epsilon or override_random:
            ...
            best_action = mcts.best_action(20)
        ...
        for episode in range(starting_episode, num_episodes):
            ...
            while not done:
                ...
                if done:
                    ...
                    reward1 = self.game.get_reward(turn1) / (1 + run * .001)
                    total_reward_agent1 += reward1
                    total_reward_agent2 -= 5
                    self.agent1.memory.append((states1[0], action1, reward1, states1[1], True))
                    break

                if run > 0:
                    reward2 = self.game.get_reward(turn2) / (1 + run * .001)
