    ...
    def train(self):
        ...
        while episode < self.num_episodes:
            ...
            while not done:
                ...
                if previous_reward is not None:
                    for k in range(4):
                        self.memory[3-turn].push(multi_channel_board_representation(rotate_board(previous_board, k)), flatten_action(rotate_action(unflatten_action(previous_action), k)), previous_reward, multi_channel_board_representation(rotate_board(board, k)), done)
                if done:
                    for k in range(4):
